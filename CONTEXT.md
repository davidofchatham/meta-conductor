...
**Filter gate**:
A rule's optional restriction selecting which posts it considers at all — its **source**. Today: post type + taxonomy/term filters (and, planned, a boundary-presence condition). Independent of the rule's main logic — a post must pass the filter gate *and* the rule's own logic to be acted on.
...
**Collision**:
Two Temporal rules naming the same term as an applied term *and* whose source sets can match the same post. Not a collision when: the rules apply to disjoint post types, or they read *different* boundary fields (independent timelines — a term driven by one timeline never contends with the same term driven by another). Detected and warned (non-blocking) at settings save; not prevented. The shipped detector keys on the cheap signals (shared term + post-type overlap) and may therefore warn on rules that actually read different fields — a tolerable false positive for a non-blocking notice.
