 ...
 * - **TermMigrator**: Taxonomy-to-taxonomy migrations (see process_taxonomy_to_taxonomy_batch)
 * - **FieldConverter**: Field type conversions (see process_field_to_field_batch)
 * - **ValueMapper**: Option value mappings (see process_map_data_conversion)
 * - **BatchProcessor**: Memory-aware batching (see calculate_batch_size, should_stop_processing)
 *
...
class DataProcessor {
     ...
     * Note: Batch size is now managed by BatchProcessor library
     */
    private const DEFAULT_BATCH_SIZE = 25;

    /**
     * Maximum execution time per batch (seconds)
     * Note: Execution time monitoring is now in BatchProcessor library
     */
    private const MAX_EXECUTION_TIME = 15;

    /**
     * Memory usage threshold (percentage)
     * Note: Memory monitoring is now in BatchProcessor library
     */
     ...
     * Constructor
     *
     * @param FieldMapper $field_mapper Field mapper instance
     */
    public function __construct( FieldMapper $field_mapper ) {
        $this->field_mapper = $field_mapper;

        // Initialize library instances
        $this->term_migrator = new TermMigrator();
        $this->field_converter = new FieldConverter();
        $this->value_mapper = new ValueMapper();
