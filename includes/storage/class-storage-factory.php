...
class StorageFactory {
     ...
     *
     * @return RuleStorage Storage implementation
     */
    private static function create_storage(): RuleStorage {
     ...
     *
     * @example
     * StorageFactory::configure(['type' => 'cpt']);
     */
     ...
     *
     * @example
     * if (StorageFactory::supports('revisions')) {
     ...
     *
     * @example
     * $stats = StorageFactory::get_statistics();
    ...
    public static function get_statistics(): array {
        ...
        if ($storage->get_storage_type() === 'options') {
            $option_value = get_option(OptionRuleStorage::OPTION_NAME, []);
     ...
     * $results = StorageFactory::migrate_storage('options', 'cpt', [
     *     'backup' => true,
     *     'dry_run' => false
     * ]);
     */
    public static function migrate_storage(string $from_type, string $to_type, array $options = []): array {
        ...
        if ($options['backup'] ?? true) {
            ...
            $current_data = get_option(OptionRuleStorage::OPTION_NAME);
        ...
        try {
            ...
            if (!($options['dry_run'] ?? false)) {
                ...
                if ($options['delete_source'] ?? false) {
                    ...
                    if ($from_type === 'options') {
                        delete_option(OptionRuleStorage::OPTION_NAME);
                    }
            ...
            if ($results['backup_created']) {
                ...
                update_option(OptionRuleStorage::OPTION_NAME, $backup_data);
