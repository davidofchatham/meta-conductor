...
class RuleEngine {
    ...
    public function __construct() {
        $this->condition_evaluator = new ConditionEvaluator();
        $this->action_executor = new ActionExecutor();
    }
    ...
    protected function get_post_entities($filters) {
        ...
        return array_map(function($post_id) {
            return new Entity('post', $post_id);
        }, $post_ids);
    }
    ...
    protected function get_term_entities($filters) {
        ...
        return array_map(function($term_id) {
            return new Entity('term', $term_id);
        }, $term_ids);
    }
    ...
    protected function get_user_entities($filters) {
        ...
        return array_map(function($user_id) {
            return new Entity('user', $user_id);
        }, $user_ids);
    }
     ...
     *
     * @since 0.2.0
     * @param Entity $source_entity The entity triggering the rule
     ...
     * @return Entity[] Array of target entities (may be empty if none found)
     *
     * @example
     * // Get all posts that have a specific term
     * $term_entity = new Entity('term', 123);
     * $rule = [
     *     'target_type' => 'post',
     *     'target_filters' => [
     ...
     *
     * @param Entity $source_entity Source entity
    ...
    protected function get_children_entities($source_entity) {
        ...
        if ($source_entity->get_type() === 'post') {
            ...
            foreach ($child_ids as $child_id) {
                $children[] = new Entity('post', $child_id);
            }
            ...
            if (!is_wp_error($child_ids)) {
                ...
                foreach ($child_ids as $child_id) {
                    ...
                    if ($child && !is_wp_error($child) && $child->parent == $source_entity->get_id()) {
                        $children[] = new Entity('term', $child_id);
                    }
                }
            }
        }
    ...
    protected function get_ancestor_entities($source_entity) {
        ...
        if ($source_entity->get_type() === 'post') {
            ...
            foreach ($ancestor_ids as $ancestor_id) {
                $ancestors[] = new Entity('post', $ancestor_id);
            }
            ...
            foreach ($ancestor_ids as $ancestor_id) {
                $ancestors[] = new Entity('term', $ancestor_id);
            }
        }
        ...
        if ($source_entity->get_type() === 'term') {
            ...
            if (!is_wp_error($descendant_ids)) {
                foreach ($descendant_ids as $descendant_id) {
                    $descendants[] = new Entity('term', $descendant_id);
                }
            }
        }
     ...
     *
     * @param Entity $term_entity Term entity
    ...
    protected function get_acf_related_entities($source_entity, $filters) {
        ...
        foreach ($related as $item) {
            if (is_object($item)) {
                if (isset($item->ID)) {
                    $entities[] = new Entity('post', $item->ID);
                } elseif (isset($item->term_id)) {
                    $entities[] = new Entity('term', $item->term_id);
                } elseif (isset($item->ID)) { // User object
                    $entities[] = new Entity('user', $item->ID);
                }
            } elseif (is_numeric($item)) {
                // Assume post ID by default
                $entity_type = $filters['related_type'] ?? 'post';
                $entities[] = new Entity($entity_type, $item);
            }
        }
