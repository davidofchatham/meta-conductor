...
(function($) {
    ...
    /**
     * Setup sub-tab switching for conversion tabs
     */
    function setupSubTabSwitching() {
        $(document).on('click', '.bws-conversion-sub-tab', function(e) {
            e.preventDefault();

            const targetTab = $(this).data('tab');

            // Update active tab
            $('.bws-conversion-sub-tab').removeClass('active');
            $(this).addClass('active');

            // Show corresponding content
            $('.bws-conversion-sub-tab-content').removeClass('active');
    ...
    function loadFieldsForContext(contentType, context, fieldType = 'source') {
        ...
        const requestData = {
            action: 'bws_meta_manager_conversion_get_fields',
            nonce: bwsMetaManager.nonce,
            context: context,
        ...
        $.post(bwsMetaManager.ajaxUrl, requestData, function(response) {
    ...
    function loadTaxonomiesForContext(selectId = null) {
        $.post(bwsMetaManager.ajaxUrl, {
            action: 'bws_meta_manager_conversion_get_taxonomies',
            nonce: bwsMetaManager.nonce
        }, function(response) {
            if (response.success) {
    ...
    function loadTaxonomyTermsForMapping(taxonomy) {
        ...
        $.post(bwsMetaManager.ajaxUrl, {
            action: 'bws_meta_manager_conversion_get_taxonomy_terms',
            nonce: bwsMetaManager.nonce,
            taxonomy: taxonomy
        }, function(response) {
            if (response.success) {
    ...
    function loadFieldOptionsForMapping(fieldKey) {
        ...
        $.post(bwsMetaManager.ajaxUrl, {
            action: 'bws_meta_manager_conversion_get_options',
            nonce: bwsMetaManager.nonce,
            field_key: fieldKey
        }, function(response) {
                        ...
                        <div class="mapping-row">
                            ...
                            <div class="target-input">
                                <select name="mappings[${escapeHtml(sourceOption.value)}]" class="regular-text mapping-select">
                                    <option value="">${bwsMetaManager.strings.skip_unmapped || 'Skip this value'}</option>
    ...
    function bindEvents() {
        ...
        $(window).on('beforeunload', function() {
            if (conversionInProgress) {
                return bwsMetaManager.strings.confirm_conversion;
            }
        });
    }
    ...
    function handleFormSubmission(e) {
        ...
        // Confirm with user
        if (!confirm(bwsMetaManager.strings.confirm_conversion)) {
            return false;
        }
	...
	function estimateConversionSize(form) {
		...
		formData.append('action', 'bws_meta_manager_conversion_estimate_size');
		formData.append('nonce', bwsMetaManager.nonce);
	
		showModal('Analyzing Conversion', 'Estimating conversion size...');
		
		$.ajax({
			url: bwsMetaManager.ajaxUrl,
			type: 'POST',
			data: formData,
			processData: false,
			contentType: false,
	...
	function processNextChunk(form, chunkStart, chunkSize = 5) {
		...
		const formData = prepareFormData(form);
		formData.append('action', 'bws_meta_manager_conversion_process_chunk');
		formData.append('nonce', bwsMetaManager.nonce);
	...
	function startStandardConversion(form) {
		...
		const formData = prepareFormData(form);
		formData.append('action', 'bws_meta_manager_conversion_process');
		formData.append('nonce', bwsMetaManager.nonce);
    ...
    function handlePreviewRequest(e) {
        ...
        // Confirm with user
        if (!confirm(bwsMetaManager.strings.confirm_preview)) {
            return false;
        }
    ...
    function generatePreview(form) {
        ...
        // Prepare data with proper field name mapping
        const formData = prepareFormData(form);
        formData.append('action', 'bws_meta_manager_conversion_preview');
        formData.append('nonce', bwsMetaManager.nonce);
