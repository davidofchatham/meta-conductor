...
- **Ride-alongs.** Delete `class-settings.php` (dead shell; takes the dormant blunt `array_merge` clobber with it) — **but rehome `flatten_conflict_overrides()` first**: as of 0.8.0 the General-tab claim overrides depend on it, so it is no longer dead code and deleting the file blindly drops the per-taxonomy default. Delete the 5 unreachable AJAX bodies on `TaxonomyManager`, close [#40](https://github.com/davidofchatham/meta-conductor/issues/40) / [#41](https://github.com/davidofchatham/meta-conductor/issues/41) (shipped in 0.7.0, still open).
