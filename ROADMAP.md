...
- **A run is a full ordered pass, not a single-rule apply.** The chosen rule picks the posts; the pass applies every enabled rule to them, in authored order — so each dispatcher's `apply()` keeps its one caller, the pass, which H13 pins. Bulk is one more provocation, so a bulk run and a save over the same rules cannot diverge.
