...
- **field_name** stays because reading a VALUE (`get_field($name, $post_id)`) is post-scoped and was never ambiguous, and because it is the fallback when a key no longer resolves (deleted field, a row imported from another site).
