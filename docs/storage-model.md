...
> **⚠️ Splitting pages was considered and rejected as the answer (2026-08-13, [ADR 0003](adr/0003-ordered-rule-list-and-dispatcher.md)).** The mechanics above are accurate; the conclusion drawn from them was not. A 4-page split doesn't shrink the hot blob — Auto-Set & Restrict would host 6 of 7 rule types today and 10 of 12 eventually — and cutting further would have to cut *inside* the term group, which is exactly where rules interact and therefore the one place a storage boundary is harmful. Cross-type clobber is already dead (every handler writes via `OptionRuleStorage::save_rule()`, a per-type merge); lost-update clobber is handled by the **version token** in the next section, which works at any page count. Outcome: **one page, three tabs**, and storage is one ordered list per **effect kind** (`term_rules`, `format_rules`). The real boundary is the effect kind, not the page. [config-pages-split plan](../.claude/plans/config-pages-split.md) is superseded.
