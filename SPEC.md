...
- V13. Inside a namespaced file, every GLOBAL class reference is leading-backslash qualified: `new \WP_Query`, `\WP_Error`, `new \DateTime`/`\DateTimeImmutable`, `catch (\Exception`, `instanceof \WP_Post`, `\WP_CLI::`, `new \stdClass`, etc. Unqualified `new WP_Query` resolves to `BWS\MetaConductor\…\WP_Query` -> runtime fatal NOT caught by php -l or the autoload harness (only fires when the method executes). NOTE: unqualified global FUNCTION calls (get_post, __, is_wp_error, wp_parse_args) are FINE — PHP auto-falls-back functions (not classes/statics/instanceof/catch) to global ns. Assert: `grep -rnE '\b(new|instanceof|catch \()\s*(WP_|DateTime|Exception|DateInterval|stdClass)' includes/ | grep -v '\\'` -> 0 (plus WP_CLI:: must be \WP_CLI::). STATIC check baked into H1 (`tests/lint.php`) covering new/instanceof/catch/extends/implements/static-call/type-hints incl `DateTimeInterface` (B4). Allowlist of global names lives in lint.php `$gclass`. DOCBLOCK refs (`@var WP_Post|WP_User`) NOT enforced — they don't affect runtime resolution; fully-qualifying them is a Phase 2b doc/PHPStan cleanup.
...
B4 | 2026-06-19 | T12b global-qualify sweep allowlist missed DateTimeInterface -> bare type hint format_date_part(DateTimeInterface $dt) resolved to BWS\MetaConductor\Handlers\DateTimeInterface -> TypeError on title-slug post-save. | qualified line 427; V13 static check baked into H1 (proven fires on bare global type hint) — covers new/instanceof/catch/extends/implements/static/type-hints, ignores docblocks
B5 | 2026-06-19 | code-review (PR#18) found bare ?DateTime RETURN type at title-slug:398 -> resolves to BWS\MetaConductor\Handlers\DateTime -> runtime fatal. V13's H1 check MISSED it: its type-hint branch required a trailing [$&|)] sigil, so a return type followed by `{` slipped. | qualified to ?\DateTime; V13 regex widened — trailing sigil replaced with negative-lookahead (?![A-Za-z0-9_\]) so the global is matched as a complete token regardless of following punctuation; proven to fire on the reverted bug. (Known residual: property type after visibility kw `public WP_Post $x` still missed — no such code exists; note for 2b.)
```
