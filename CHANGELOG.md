...
- The shape repairs that ran on every settings-page load and every rule read: the hierarchical direction/expansion pair rewritten to the single *inheritance behavior* choice, scalar related-rule term ids rewritten to lists, and the ACF-reference key rename (`source_taxonomy`/`bidirectional`/`conflict_handling` to `taxonomy`/`keep_in_sync`, plus an invented `holder_role` default). No stored rule was still in those shapes. The settings page still fills in a missing row title.
...
- **An ACF-reference rule now points at the field you picked, even when another field shares its name.** Rules stored only the field name, and ACF resolves a bare name to whichever match it finds first — then caches that for the request — so on a site with two same-named relationship fields a rule could intermittently read the wrong field's configuration. Symptoms: a reverse lookup that found no partner and silently fell back to the slow scan, and posts skipped because the wrong field's target post types were checked.

  Rules now store and resolve by field key. Existing rules upgrade in place on the next admin page load, **except** one whose name matches more than one field, which is left as-is rather than guessed at: re-pick its field — every option in both field dropdowns now names its field group. The same-name warning is gone from the *Monitored relationship field* help text.

- **A title/slug rule's first rename of a post no longer leaves a revision.** Suppression only applied once the post already had a revision history.
...
- The per-handler bulk path — `process_existing_posts()` with its time-based and title/slug overrides, and `TitleSlugHandler::preview_rule()` — superseded by the existing-posts applier.
- Four unused admin AJAX endpoints: `bws_validate_acf_field`, `bws_get_acf_fields`, `bws_title_slug_preview`, `bws_title_slug_process_existing`.
