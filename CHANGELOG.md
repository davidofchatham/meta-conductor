...
- **The `mc-rules` fixture's only cron coverage was a false green** (dev tooling only — not shipped). `verify.php` A7 asserted `wp_next_scheduled( 'bws_taxonomy_manager_cleanup' )` was truthy. The event is scheduled at plugin load and stays scheduled under `DISABLE_WP_CRON` — which disables only the page-load spawner, not the schedule — so the check passed on a site where `cleanup_expired_rules()` had never run.
  ...
  Two handler defects surfaced while writing it, **neither fixed here**, both sharing one root cause — `TimeBasedHandler` records no per-post provenance and removes its target term unconditionally when out of range. [#71](https://github.com/davidofchatham/meta-conductor/issues/71) is the save path: `apply_time_based_rule()`'s trailing `elseif` strips the term whoever applied it, so an out-of-range rule undoes an in-range rule's apply inside a single `process_post()` loop, and from `{TODAY+10}` the fixture's future-dated rule can never land its term at all. [#69](https://github.com/davidofchatham/meta-conductor/issues/69) is the cron path: `cleanup_expired_rules()` has no already-cleaned flag and re-runs daily forever, so it strips what an active rule sharing the target applies. Recorded as §6f and §6g in `tools/fixtures/handler-fixture-matrix.md`; #71 is what forces A7 to synthesize a rule set rather than only slide dates.
