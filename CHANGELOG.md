...
- **⚠️ A *bidirectional* related rule now removes its target term whenever no trigger term is present — not only at the moment a trigger is removed** (**#61**). The old apply read the before/after term IDs `set_object_terms` hands out, so it could remove the target only when it saw a trigger *leave* in that particular write. An ordered pass hands over rules, not before/after pairs — deliberately, since a rule that consumes an earlier rule's write has no such pair for it — so the question becomes a question about the post's current state: trigger present, apply; trigger absent, remove.

  **What changes for you.** A post that carries the target term but has never carried a trigger used to keep it indefinitely, because the removal signal never arrived. It now loses that term on the next save or pass. That is what "bidirectional" was always meant to say — the target mirrors the trigger — and it is the same retroactive ownership a date-window rule has always had over its own target ([ADR 0001](docs/adr/0001-temporal-rule-general-model-constrained-ui.md)). The toggle's description says so now.

  **Rules with *Bidirectional* switched off are unaffected**, and it is off by default: they never removed anything and still do not. If you have a bidirectional rule whose target term is also applied by hand or by another rule, either turn the toggle off or give that rule its own target — the two arrangements were indistinguishable before and are not now.

  **A rule whose trigger no longer exists does nothing at all**, rather than removing. A deleted trigger term or a taxonomy that stopped being registered makes "is the trigger present?" unanswerable, not answerable with *no* — and under the old model the question never arose, because a term that does not exist generates no removal signal. A rule listing several trigger terms still works off whichever of them survive.
- **A rule that would not change a post's terms no longer writes them** (**#62**). What *owning* / *contributing* / *deferring* mean is now computed in one place and used by both the write itself and the check that decides whether the write is worth making — they were two parallel copies of the same three-way decision, which is a correctness risk rather than untidiness (a drift in either direction silently loses a rule's effect or writes on every save). The visible effect is fewer redundant term writes: applying terms a post already carries is now a no-op instead of a rewrite to the same value.
- **⚠️ Two *terms from a referenced post (ACF)* rules in the same taxonomy now compose by their position in the list, instead of being merged into one result** (**#63**). This rule type was the last one still running all of its rules at once from its own hooks, which meant it occupied a single position in the ordered list however many rules it held — so two of its rules with another rule type's between them was not something you could express, and their combined effect was fixed. Each rule is now applied in its own turn, like every other rule in the list.

  **What changes for you.** If two of these rules feed the same taxonomy on the same post and both have *Keep in sync* on, the **lower one in the list wins** — *Keep in sync* means "only this rule's terms belong here", and the rule that runs last is the one that gets to say so. Previously the two were unioned. If you want both sets, turn *Keep in sync* off on the second one (add-only rules never remove), or reorder them so the owning rule runs first. **One rule with several source posts is unaffected** — a rule still unions its own sources; this is only about two separate rules.

  For the same reason, a dependent that loses its last source under a *Keep in sync* rule is now emptied by that rule even if another, add-only rule still has sources for it. That add-only rule re-adds its own contribution when it runs, so put it below the keep-in-sync one.

  **Everything else about this rule type behaves as before**: severing a relationship at either end still withdraws the source's terms, deleting a source post still cleans up its dependents, the reverse-field and add-only paths are unchanged.
- **A relationship edit made by code alone now reconciles** (**#63**). A bare `update_field()` — no post save, no ACF form — used to record the severed link and then never act on it, so the dependent kept a term whose source was gone until somebody re-saved the post. It is now reconciled in the same pass as everything else. Imports and CLI scripts no longer need to fire `acf/save_post` afterwards to flush.
- **Rules for every rule type that writes terms now run in the order you authored them, on every path** (**#63** completes **#53**). With this last conversion, all six term rule types are executed by the central dispatcher — one ordered pass per post, whether the post was saved in the editor, edited in bulk, touched by cron, or written to by another plugin through ACF.

### Added

- **The order you put rules in is now the order they run** (**#60**, [ADR 0003](docs/adr/0003-ordered-rule-list-and-dispatcher.md)). A central **term dispatcher** owns the hooks that used to belong to individual rule types, and any save runs **one full ordered pass** over the post: every enabled term rule, top to bottom, each recomputing from the post's current state and doing nothing if nothing changed. Drag a rule above another and the result changes — which is what the ordered list was for.

  Two rule types make the move in this release, **hierarchy inheritance** and **level restriction**. They are the pair whose interaction was least predictable, because the order between them was previously set by hook priorities nobody could see: level restriction registered at priority 5 specifically to land before hierarchy's 10. On a post carrying a deep term and a sibling of one of its ancestors, "inherit ancestors, then restrict" and "restrict, then inherit ancestors" genuinely differ — and now you choose which. The remaining five rule types keep their existing behaviour until their own conversion (#61–#64).

  **What you may notice.** Rules now apply once at the end of a save rather than the instant a term is written. The editor still shows the result — the pass runs before the response is built — but code reading terms back immediately after a programmatic `wp_set_object_terms()` in the *same* request will see the pre-rule value, where previously the expansion had already happened inline. The Admin Columns inline edit, whose response is built earlier still, asks for its pass directly, so that column is accurate as before.

  **Turning it off works the way it already did.** Everything that stands the ACF reapply path down — `meta_conductor_acf_reapply_enabled`, a WordPress import, the conversion tool's bulk field writes — now also stands the pass down, because they all mean the same thing: do not recompute rules for this post. A new `meta_conductor_term_pass_enabled` filter is the finer control for a site that wants one without the other.

  **Bulk "apply to existing posts" now runs a pass too**, for the converted types, so it produces the same result a save would rather than applying one rule type in isolation. The same holds for the ACF-only write paths (a bare `update_field()`, a REST field write): they mark the post and the pass runs with everything else.

  Under the hood the four per-handler re-entrancy booleans start coming out. Re-entry is now a single **pass-scoped lock** keyed to *(post, effect kind)*: a rule's own write cannot restart the pass it is inside, but it stays visible to every rule after it — which is exactly what makes order the composition mechanism. A write to a *different* post still gets that post its own pass in the same drain.

- **Date-window and related-term rules joined the ordered pass** (**#61**). Both now register no hooks of their own; the dispatcher runs them in the position you gave them, alongside hierarchy inheritance and level restriction. Two of the five remaining types are done; propagation and terms-from-a-referenced-post follow (#62, #63).

  **The daily expiry sweep runs an ordered pass now, not a private removal.** When a date window closes, the nightly job used to strip the term from each matching post by itself — one rule, executed alone, outside any pass. It now selects those posts and hands them to the dispatcher, so the whole list runs over each one exactly as a save would. A term a date-window rule writes at 3am is therefore visible to every rule below it in the list, on the same path an editor save takes; before, it was visible to nothing until somebody re-saved the post. What the sweep *removes* is unchanged: the expired rule's own out-of-range branch still strips its target term from a post outside the window, whoever put it there ([ADR 0001](docs/adr/0001-temporal-rule-general-model-constrained-ui.md)).

  One thing about the sweep *did* change, and it is a correction: it now honours the rule's **Limit to post statuses**. The old nightly run consulted no gate at all, so a rule scoped to published posts still stripped its term from drafts overnight while a save of that same draft left it alone. Cron and save now agree.

  Publishing a post still triggers the date-window path, and a related rule still applies its target when its trigger term is written — through the taxonomy box or through an ACF taxonomy field. New behaviour sweep `tools/fixtures/mc-rules/sweep-61-appliers.php` covers all of it, including an order control: move the consuming rule above the producing one and the consumption stops.
- **Propagation joined the ordered pass, and it now works by PULL** (**#62**, closes **#35**). This is the one conversion that changes how a rule reaches a second post, so it is worth saying plainly.

  **Before.** Saving a parent walked every descendant and wrote each one directly. The written child never got a rule pass of its own — one rule reached it, out of the order you authored, and whatever else was listening then ran against that write on its own hooks. That is the bug behind **#35**: with a propagation rule and a *descendants* hierarchy rule on the same taxonomy, children ended up with a level of terms nobody had asked any rule for.

  **Now.** Each post reconciles ITSELF against its parent, and the parent's change simply marks its children for their own full ordered pass. Same posts, same terms, but every one of them is reconciled by the whole list, in your order — so propagation and hierarchy inheritance compose on a child exactly as they compose on the post you edited. On a three-level chain the passes run one level at a time and settle instead of gaining a level per generation.

  **You can now choose the outcome, and the two orders genuinely differ.** Propagation above hierarchy inheritance: the child receives its parent's terms and the hierarchy rule then expands them **once**. Hierarchy inheritance above propagation: the child receives its parent's terms with **no** expansion, because the hierarchy rule ran before they arrived. Both are legitimate; the list is where you say which you want. This also removes the last piece of hidden ordering between those two rule types, which used to be decided by the sequence the plugin happened to construct its handlers in.

  **Removing a term from a parent still removes it from descendants**, including a removal made programmatically with `wp_remove_object_terms()`, which fires none of the usual save hooks. That case cannot be answered from the current state of things — once the parent no longer has the term, nothing distinguishes it from a term the child was given independently — so the removal is recorded at the moment it happens and each child subtracts it. Two consequences worth knowing: a term removed and re-added to the parent in the same request is **kept** on the children (previously it was dropped), and a parent whose ACF mirror field lags its native terms within one request can no longer bounce the removed term back down.

  **One correction rides along, in the same spirit as #61's cron/save one.** A propagation rule's **Limit to post statuses** now applies to the post being written, so a rule scoped to published posts no longer copies terms onto draft children. Previously the descendant walk consulted no status gate at all, so the same rule behaved differently depending on which post you saved. Rules that leave the setting empty — the default, meaning every status — are unaffected.

  Four of the seven rule types now run in your order; *terms from a referenced post* is the last term type left (**#63**). New behaviour sweep `tools/fixtures/mc-rules/sweep-62-propagation.php` covers the chain, the removal, and the #35 pair in both orders — including a count proving each level is reconciled exactly once.
- **A rule that affects other posts declares them; it no longer writes them** (**#62**). New `fan_out()` seam on the handler base, empty for every rule type but propagation. A handler returns the posts one step away, the dispatcher marks them, and each gets its own pass — so the recursion lives in the queue, where it is bounded, rather than in a handler walking a subtree. H13 asserts a declared fan-out writes nothing.
- **A post can now be reconciled more than once in a single drain** (**#62**). Nothing orders the queue, so a bulk edit or an ACF flush can hand the dispatcher a child before its parent; with a strict one-pass-per-post rule the parent's fan-out would then be discarded as "already done", leaving the child settled against the parent's pre-pass state. Passes are idempotent, so the bound is now three per post per drain rather than one. In the ordinary case — a save, or a tree reconciled from its root — nothing fans upward and every post is still passed exactly once.
...
- **Harness H13 — `tests/verify-term-dispatcher.php`** (source inspection, host PHP, no WP). The dispatcher's correctness is hook registrations, call-site counts and control-flow gates, none of which has a runtime-observable signature until it is already wrong on a live site — a handler that kept a hook just runs twice, out of order; a stray re-entrancy boolean just silences a chain. So they are pinned by inspection, like the ACF write queue's invariants (H8). It asserts the trigger union is mark-only, the drain runs after the ACF queue's flush, the lock is keyed on both entity and kind, and `apply_to_post` has exactly **one** call site in the whole plugin. It also enumerates which rule types are converted and which still own their hooks, and fails if a handler's registrations disagree with the side it is listed on — so each conversion ticket shrinks a list visible in its diff and cannot shrink it without doing the work. **#62** added three more: a hook a converted handler is still allowed to register must have a callback that *records* and never writes, the declared fan-out must be collected by the pass and enqueued rather than executed, and a handler's own `fan_out()` must write nothing. Negative-tested against seven mutations.
...
- **Level restriction no longer replaces a post's terms with the contents of its ACF field** (**#60**). When a level-restriction rule found an ACF taxonomy field to prune, it pruned the field *and then overwrote the post's whole taxonomy with the result*. That was survivable while it only ran on an ACF save, where the field was the authoritative source for the save in hand. Inside an ordered pass it is not: the rule runs on every pass, so that overwrite would wipe terms a rule earlier in the list had just added — destroying them rather than pruning them, on a rule whose entire job is to remove only what violates its constraint. It now prunes the field and leaves the taxonomy to the native half of the same rule, which restricts the post's live terms under the same setting.
