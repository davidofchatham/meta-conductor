...
- The unused generic rule engine (`RuleEngine`, `ConditionEvaluator`, `ActionExecutor`, and the `Entity` wrapper only it constructed) and the handler-base methods that fed it: `process_rule()`, `process_all_rules()`, `bulk_process()`, `process_post()`, `get_statistics()`, `clear_cache()`, `get_defaults()`, `convert_legacy_rule()`, and the run logging behind them. No rule type had used it since the dispatcher took over, and its rule-shape check silently rejected every related-post-terms and title/slug rule (#26). The `bws_meta_conductor_before_process_*` / `after_process_*` / `clear_*_cache` actions no longer fire.
- **The plugin's three database tables** (`bws_meta_conductor_log`, `bws_relationship_log`, `bws_batch_queue`). Nothing wrote to or read any of them — the log's only writer went with the rule engine, and the other two never had one. Activation no longer creates them, and the update drops them. Uninstall previously left the last two behind; it now drops every table the plugin has ever created.
