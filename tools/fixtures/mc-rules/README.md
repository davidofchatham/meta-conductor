...
| `sweep-apply-existing.php` | FW-16 04 behaviour sweep: `ExistingPostsApplier::run_batch()` over a rule's reach. Stepped — `enabled` (end state = re-save) / `disabled` (one-time run: storage untouched, a later save neither undoes nor re-applies) / `stale` (refused, nothing written) / `off` (pass switched off → error, nothing written) / `fanout` (propagation children outside the reach reconciled in the same batch, not reported) / `rpt` (draft dependent of a published source reached) / `revisions` (a format run leaves no revision; turns revision support on for `mc_item` for its request) / `restore` (lists `mc_item` slugs). Each step snapshots every mc_item / mc_section post and puts it back, and holds passes down for the rest of its request so the shutdown ACF flush cannot re-pass the put-back. |
