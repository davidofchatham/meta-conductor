...
class BWS_ACF_Field_Mapper {
    ...
    public function get_field_groups( bool $force_refresh = false ): array {
        $cache_key = 'bws_acf_conversion_field_groups';
    ...
    public function get_fields_by_type( bool $force_refresh = false ): array {
        $cache_key = 'bws_acf_conversion_fields_by_type';
    ...
    public function get_taxonomies( bool $force_refresh = false ): array {
        $cache_key = 'bws_acf_conversion_taxonomies';
    ...
    public function get_taxonomy_fields( string $taxonomy, bool $force_refresh = false ): array {
        $cache_key = "bws_acf_conversion_taxonomy_fields_{$taxonomy}";
    ...
    public function validate_field_conversion( string $source_field_key, string $target_field_key ): array {
        ...
        if ( ! $source_field ) {
            $result['errors'][] = __( 'Source field not found.', 'acf-data-conversion' );
            return $result;
        }

        if ( ! $target_field ) {
            $result['errors'][] = __( 'Target field not found.', 'acf-data-conversion' );
            return $result;
        }
        ...
        if ( ! $source_field['supported'] ) {
            $result['errors'][] = sprintf(
                ...
                __( 'Source field type "%s" is not supported for conversion.', 'acf-data-conversion' ),
                $source_field['type']
            );
        }
        ...
        if ( ! $target_field['supported'] ) {
            $result['errors'][] = sprintf(
                /* translators: %s: field type */
                __( 'Target field type "%s" is not supported for conversion.', 'acf-data-conversion' ),
                $target_field['type']
            );
        }
    ...
    private function check_conversion_risks( array $source_field, array $target_field, array &$result ): void {
        ...
        if ( in_array( $source_field['type'], $complex_types, true ) && 
            ...
            $result['warnings'][] = __( 'Converting from complex field type to simple type may result in data loss.', 'acf-data-conversion' );
        }
        ...
        if ( in_array( $source_field['type'], $multiple_types, true ) && 
            ...
            $result['warnings'][] = __( 'Converting from multiple-value field to single-value field may result in data loss.', 'acf-data-conversion' );
        }

        // Numeric to text conversions
        if ( 'number' === $source_field['type'] && 'text' !== $target_field['type'] ) {
            $result['warnings'][] = __( 'Converting from number field may require validation in target field.', 'acf-data-conversion' );
        }
        ...
        if ( $source_field['has_options'] && $target_field['has_options'] ) {
            ...
            if ( ! empty( $missing_options ) ) {
                $result['warnings'][] = sprintf(
                    ...
                    __( 'Target field is missing these options: %s', 'acf-data-conversion' ),
    ...
    public function create_taxonomy_term( string $taxonomy, string $term_name, array $args = [] ) {
        if ( ! taxonomy_exists( $taxonomy ) ) {
            return new WP_Error( 'invalid_taxonomy', __( 'Taxonomy does not exist.', 'acf-data-conversion' ) );
        }
    ...
    public function clear_cache(): void {
        ...
        if ( function_exists( 'wp_cache_delete_group' ) ) {
            wp_cache_delete_group( 'bws_acf_conversion' );
        }
    }
    ...
    public function validate_taxonomy_mapping( string $field_key, string $taxonomy ): array {
        ...
        $field = $this->get_field_by_key( $field_key );
        if ( ! $field ) {
            $result['errors'][] = __( 'Field not found.', 'acf-data-conversion' );
            return $result;
        }

        if ( ! taxonomy_exists( $taxonomy ) ) {
            $result['errors'][] = __( 'Taxonomy does not exist.', 'acf-data-conversion' );
            return $result;
        }
        ...
        if ( ! $field['taxonomy_mappable'] ) {
            $result['errors'][] = sprintf(
                /* translators: %s: field type */
                __( 'Field type "%s" cannot be mapped to taxonomy.', 'acf-data-conversion' ),
                $field['type']
            );
            return $result;
        }
        ...
        if ( in_array( $field['type'], $multiple_value_types, true ) && ! $taxonomy_obj->hierarchical ) {
            $result['warnings'][] = __( 'Mapping multiple values to non-hierarchical taxonomy may create many terms.', 'acf-data-conversion' );
        }
