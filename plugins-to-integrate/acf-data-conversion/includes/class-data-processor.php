<?php
/**
 * ACF Data Processor Class - UPDATED VERSION
 * 
...
class BWS_ACF_Data_Processor {

    /**
     * Field mapper instance
     *
     * @var BWS_ACF_Field_Mapper
     */
     ...
     */
    public function __construct( BWS_ACF_Field_Mapper $field_mapper ) {
    ...
    public function process_copy_data_conversion( array $config, bool $dry_run = false ): array {
        ...
        if ( empty( $items ) ) {
            $result['message'] = __( 'No items found to copy.', 'acf-data-conversion' );
            return $result;
        }
        ...
        foreach ( $batches as $batch_index => $batch_items ) {
            ...
            if ( $this->should_stop_processing() ) {
                ...
                $result['message'] = __( 'Processing stopped due to resource limits.', 'acf-data-conversion' );
                break;
            }
        }
    ...
    public function process_map_data_conversion( array $config, bool $dry_run = false ): array {
        ...
        if ( empty( $items ) ) {
            $error_message = sprintf(
                __( 'No items found with data in the source field. Checked field "%s" (%s) in %d %s.', 'acf-data-conversion' ),
...
public function process_conversion_chunk( array $config, bool $dry_run = false, int $chunk_start = 0, int $chunk_size = 5 ): array {
    ...
    if ( ! $session_data && $chunk_start === 0 ) {
        ...
        if ( ! $session_data ) {
            $result['errors'] = [ __( 'Failed to initialize conversion session.', 'acf-data-conversion' ) ];
            return $result;
        }
    }
    
    if ( ! $session_data ) {
        $result['errors'] = [ __( 'Invalid session. Please restart the conversion.', 'acf-data-conversion' ) ];
        return $result;
    }
...
public function cleanup_old_sessions(): void {
    ...
    private function validate_source_field_enhanced( array $config ): array {
        ...
        if ( empty( $config['source_field'] ) ) {
            $result['errors'][] = __( 'Source field is required.', 'acf-data-conversion' );
            return $result;
        }
        ...
        if ( ! empty( $config['mappings'] ) && ! $source_field_data['has_options'] ) {
            $result['errors'][] = sprintf(
                __( 'Source field "%s" (%s) does not support options. Only select, checkbox, radio, and button_group fields can be mapped.', 'acf-data-conversion' ),
            ...
            try {
                ...
                if ( $conversion_result['success'] ) {
                    ...
                    $batch_result['errors'][] = sprintf(
                        ...
                        __( '%1$s %2$d: %3$s', 'acf-data-conversion' ),
    ...
    private function process_map_data_batch( array $items, array $config, bool $dry_run, int $batch_number ): array {
        ...
        foreach ( $items as $item ) {
            ...
            // Check if we should stop early
            if ( $this->should_stop_batch_processing( $batch_result['start_time'] ) ) {
                $batch_result['warnings'][] = __( 'Batch stopped early due to resource limits.', 'acf-data-conversion' );
                break;
            }
        }
    ...
    private function execute_copy_data_conversion( array $item, array $config, bool $dry_run ): array {
        ...
        switch ( $copy_type ) {
            ...
            default:
                return [
                    'success' => false,
                    'error' => __( 'Invalid copy type.', 'acf-data-conversion' )
                ];
        }
    }
    ...
    private function execute_map_data_conversion( array $item, array $config, bool $dry_run ): array {
        ...
        if ( ! $source_field_data ) {
            return [
                'success' => false,
                'error' => __( 'Source field not found.', 'acf-data-conversion' )
            ];
        }
        ...
        if ( empty( $source_value ) ) {
            return [
                'success' => true,
                'message' => __( 'No data to map.', 'acf-data-conversion' )
            ];
        }
    ...
    private function copy_field_to_field( array $item, array $config, bool $dry_run ): array {
        ...
        if ( ! $source_field_data || ! $target_field_data ) {
            return [
                'success' => false,
                'error' => __( 'Field data not found.', 'acf-data-conversion' )
            ];
        }
        ...
        if ( empty( $source_value ) && ! is_numeric( $source_value ) ) {
            return [
                'success' => true,
                'message' => __( 'No data to copy.', 'acf-data-conversion' )
            ];
        }
        ...
        if ( ! $dry_run ) {
            ...
            if ( ! $update_result ) {
                return [
                    'success' => false,
                    'error' => __( 'Failed to update field value.', 'acf-data-conversion' )
                ];
            }
        }
    ...
    private function copy_field_to_taxonomy( array $item, array $config, bool $dry_run ): array {
        ...
        if ( empty( $term_names ) ) {
            return [
                'success' => true,
                'message' => __( 'No valid terms extracted.', 'acf-data-conversion' )
            ];
        }
    ...
    private function copy_taxonomy_to_field( array $item, array $config, bool $dry_run ): array {
        ...
        if ( ! $target_field_data ) {
            return [
                'success' => false,
                'error' => __( 'Target field not found.', 'acf-data-conversion' )
            ];
        }
        ...
        if ( is_wp_error( $terms ) || empty( $terms ) ) {
            return [
                'success' => true,
                'message' => __( 'No terms to copy.', 'acf-data-conversion' )
            ];
        }
    ...
    private function copy_taxonomy_to_taxonomy( array $item, array $config, bool $dry_run ): array {
        if ( $config['content_type'] !== 'posts' ) {
            return [
                'success' => false,
                'error' => __( 'Taxonomy to taxonomy copying only supported for posts.', 'acf-data-conversion' )
            ];
        }
    ...
    private function map_to_field( array $item, $source_value, array $config, bool $dry_run ): array {
        ...
        if ( ! $dry_run && ! empty( $new_values ) ) {
            ...
            if ( ! $update_result ) {
                return [
                    'success' => false,
                    'error' => __( 'Failed to update target field.', 'acf-data-conversion' )
                ];
            }
        }
    ...
    private function validate_copy_data_config( array $config ): array {
        ...
        if ( empty( $config['content_type'] ) || ! in_array( $config['content_type'], [ 'posts', 'taxonomy_terms' ], true ) ) {
            $errors[] = __( 'Valid content type is required.', 'acf-data-conversion' );
        }

        if ( empty( $config['copy_type'] ) || ! in_array( $config['copy_type'], self::COPY_TYPES, true ) ) {
            $errors[] = __( 'Valid copy type is required.', 'acf-data-conversion' );
        }
        ...
        switch ( $config['copy_type'] ?? '' ) {
            case 'field_to_field':
                if ( empty( $config['source_field'] ) ) {
                    $errors[] = __( 'Source field is required.', 'acf-data-conversion' );
                }
                if ( empty( $config['target_field'] ) ) {
                    $errors[] = __( 'Target field is required.', 'acf-data-conversion' );
                }
                break;
            ...
            case 'field_to_taxonomy':
                ...
                if ( empty( $config['target_taxonomy'] ) ) {
                    $errors[] = __( 'Target taxonomy is required.', 'acf-data-conversion' );
                }
                break;
                
            case 'taxonomy_to_field':
                if ( empty( $config['source_taxonomy'] ) ) {
                    $errors[] = __( 'Source taxonomy is required.', 'acf-data-conversion' );
                }
                if ( empty( $config['target_field'] ) ) {
                    $errors[] = __( 'Target field is required.', 'acf-data-conversion' );
                }
    ...
    private function validate_map_data_config( array $config ): array {
        ...
        if ( empty( $config['mappings'] ) || ! is_array( $config['mappings'] ) ) {
            $errors[] = __( 'Option mappings are required.', 'acf-data-conversion' );
        }

        if ( empty( $config['target_type'] ) || ! in_array( $config['target_type'], [ 'field', 'taxonomy' ], true ) ) {
            $errors[] = __( 'Valid target type is required.', 'acf-data-conversion' );
        }

        if ( $config['target_type'] === 'field' && empty( $config['target_field'] ) ) {
            $errors[] = __( 'Target field is required when mapping to field.', 'acf-data-conversion' );
        }

        if ( $config['target_type'] === 'taxonomy' && empty( $config['target_taxonomy'] ) ) {
            $errors[] = __( 'Target taxonomy is required when mapping to taxonomy.', 'acf-data-conversion' );
        }
